class SnowFall {
  ...
  initCanvas() {
    ...
    snowcanvas.style.cssText = 'position: fixed; top: 0; left: 0; z-index: 1; pointer-events: none;';
...
class FlakeMove {
  ...
  reset(initial = false) {
    ...
    this.y = initial ? Math.floor(Math.random() * this.canvasHeight) - this.canvasHeight : -this.maxSize;
  ...
  update() {
    ...
    if (this.x > this.canvasWidth + this.maxSize || this.x < -this.maxSize || this.y > this.canvasHeight) {
      this.reset();
    }
  }
  ...
  render(ctx) {
    ...
    gradient.addColorStop(.5, 'rgba(255, 255, 255, 0.5)'); // 中心白色
